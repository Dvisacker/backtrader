...
def plot_rolling(df):
    ...
    ax[0].plot(df.index, df.close, label='raw close', lw=1.)
    ax[0].plot(df.close.rolling(window=12).mean(), label="rolling mean", lw=1.)
    # ax[0].plot(df.close.rolling(window=12).std(), label="rolling std (x10)", lw=1.)
    ax[0].legend()

    ax[1].plot(df.index, df.z_close, label="de-trended close", lw=1.)
    ax[1].plot(df.z_close.rolling(window=12).mean(), label="rolling mean", lw=1.)
    ax[1].plot(df.z_close.rolling(window=12).std(), label="rolling std (x10)", lw=1.)
    ax[1].legend()

    ax[2].plot(df.index, df.zp_close, label="12 lag differenced de-trended close", lw=1.)
    ax[2].plot(df.zp_close.rolling(window=12).mean(), label="rolling mean", lw=1.)
    ax[2].plot(df.zp_close.rolling(window=12).std(), label="rolling std (x10)", lw=1.)
    ax[2].legend()

...
def add_barriers_on_buy_sell_signals(close, side, stop_thresholds):
    ...
    #3) we replace NaT by a very large date in order to compare values
    df0['take_profit'][pd.isnull(df0['take_profit'])] = pd.Timestamp(2100, 1, 1, 1).tz_localize('UTC').tz_convert('US/Eastern')
    df0['stop_loss'][pd.isnull(df0['stop_loss'])] = pd.Timestamp(2100, 1, 1, 1).tz_localize('UTC').tz_convert('US/Eastern')
